...
## Testing Guidelines
Add or update tests for every behavior change. Place package-level Go tests beside the code they cover, and broader server or admin coverage under `tests/server/*_test.go`. If you change `internal/client/dashboard/ui-v2`, rebuild `internal/client/dashboard/ui-v2/dist` in the same change so embedded assets stay in sync.
