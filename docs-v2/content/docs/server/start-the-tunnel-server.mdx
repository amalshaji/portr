...
| `PORTR_PROXY_PORT` | Tunnel proxy and WebSocket endpoint port | `8001` |
...
The server process exposes two HTTP listeners:

- `PORTR_ADMIN_PORT` serves the admin dashboard and API, including `/api/v1/healthcheck`.
- `PORTR_PROXY_PORT` serves the WebSocket tunnel endpoint at `/_portr/tunnel/connect`
  and routes wildcard tunnel traffic such as `https://my-app.example.com`.

The production compose file routes `PORTR_DOMAIN` to the admin listener and
`*.PORTR_DOMAIN` to the tunnel listener. Clients receive both `server_url` and
`ws_url` in their generated config so these paths can be routed independently.
