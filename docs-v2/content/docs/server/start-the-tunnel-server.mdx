...
PORTR_WS_URL=tunnel.example.com
...
| `PORTR_SSH_HOST_KEY` | Stable PEM-encoded Ed25519 host private key when `PORTR_TRANSPORT=ssh` | Required for SSH |
| `PORTR_WS_URL` | WebSocket control hostname routed to the tunnel proxy when `PORTR_TRANSPORT=websocket` | `localhost:8001` |
...
When using the default `ssh` transport, generate the host key once and keep the
same value across deployments. Replacing it changes the server identity and
causes returning clients to report an SSH host-key mismatch.
...
You can also generate a key with Portr:
...
<Callout type="info">
  Store the generated key as a persistent multi-line environment value or
  Docker secret. Do not generate a new key each time the server starts.
</Callout>
...
For WebSocket transport, set `PORTR_WS_URL` to a hostname covered by the
wildcard route, such as `tunnel.example.com`. Generated configs keep
`tunnel_url: example.com` separate so public application URLs remain
