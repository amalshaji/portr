---
title: Server Setup
description: Guide to setting up the self-hosted Portr server, covering the admin dashboard, tunnel backend, DNS, and TLS certificate configuration.
---
...
- **Port 2222**: Open for SSH tunnel clients when `PORTR_TRANSPORT=ssh`
- **Port 8001**: Open for wildcard proxied HTTP traffic, and for WebSocket tunnel clients when `PORTR_TRANSPORT=websocket`
