---
title: App Server
description: Run the Portr CLI as a local HTTP API to create, manage, and stop tunnels programmatically from your own scripts and applications.
---
...
the regular CLI commands. It reads your configured `server_url`, `transport`,
`ssh_url` or `ws_url`, `tunnel_url`, `secret_key`, request logging settings, and
health check settings from the Portr client config file.
