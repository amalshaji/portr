...
description: Learn how to start a WebSocket tunnel with the Portr CLI to expose local real-time apps and WebSocket servers to the public internet.
---
...
```bash
portr http 9000 -s realtime-dev
```
...
The Portr client-to-server tunnel transport is selected by your server-generated
config. It connects to `ssh_url` when `transport: ssh`, or to `ws_url` when
`transport: websocket`. Public HTTP and application WebSocket traffic still uses
the normal tunnel URL such as `https://realtime-dev.example.com` or
`wss://realtime-dev.example.com`.
...
5. **Local logging**: The inspector records the handshake and WebSocket frame events in the local SQLite dashboard database
