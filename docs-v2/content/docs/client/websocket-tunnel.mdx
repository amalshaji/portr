...
The Portr client-to-server tunnel itself also uses WebSockets. The client connects
to `ws_url` from your config, while public HTTP and application WebSocket traffic
uses the normal tunnel URL such as `https://realtime-dev.example.com` or
