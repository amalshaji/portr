...
HTTP tunnels use the server-selected tunnel transport from your config: SSH when
`transport: ssh`, or WebSocket when `transport: websocket`. By default, each
HTTP tunnel starts two workers so concurrent incoming requests can keep moving
while another request is in flight. Configure `pool_size` on a tunnel template
when you want a different worker count:
...
2. Opens tunnel workers to the configured `ssh_url` or `ws_url`, based on `transport`
