...
For one-off runs, you can override the config with CLI flags such as:

```bash
portr http 3000 --dashboard-port 8888
portr start --disable-dashboard
```

`disable_dashboard` takes precedence over `dashboard_port`, so a disabled dashboard stays disabled until you change the disable setting itself.
