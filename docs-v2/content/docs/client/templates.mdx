...
- **ws_url**: The WebSocket tunnel endpoint URL for client tunnel connections. Defaults to `server_url` if omitted.
...
- **pool_size**: HTTP only. Number of WebSocket workers to run per HTTP tunnel (default: 2). Increases resilience and throughput.

<Callout type="info">
  `server_url` and `ws_url` are often the same host. Set them separately when your
  admin/API traffic and tunnel WebSocket traffic are routed through different
  hosts or ports.
</Callout>
