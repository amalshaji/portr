...
<Callout type="warn">
  If you enable host key verification, the server must present the same host key across restarts. Set `PORTR_SSH_HOST_KEY` on the server to a persistent PEM-encoded key; see [Generate SSH Host Key](/docs/server/start-the-tunnel-server#generate-ssh-host-key). Without it the server generates a new in-memory key on every start, and clients that verify host keys will refuse to connect after a restart or redeploy.
</Callout>
