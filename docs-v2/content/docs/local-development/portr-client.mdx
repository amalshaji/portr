...
description: Set up the Portr client CLI for local development, so you can build, run, and contribute changes to the tunnel client on your machine.
---
...
assume the binary is available on your `PATH`. From the repository root, you can
do that during local development with `export PATH="$PWD:$PATH"`.
...
- **Tunnel manager**: Handles WebSocket connections and port forwarding
...
`server_url` points at the admin/API server. `ssh_url` points at the SSH tunnel
listener. In local development with `portrd start all`, those are usually
`localhost:8000` and `localhost:2222`. If the server is configured with
`PORTR_TRANSPORT=websocket`, use `transport: websocket` and
`ws_url: localhost:8001` instead.
...
portr -c client.dev.yaml http 8080 -s test
