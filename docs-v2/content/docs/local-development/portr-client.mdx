...
# Emit full stored request records as JSON, including request and response payloads
...
- `--json` emits the full stored record, including headers, request body, response headers, response body, status code, replay metadata, and timestamp.
